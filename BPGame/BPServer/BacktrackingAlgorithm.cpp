 ...
 */
int** BacktrackingAlgorithm::findPath(int elementsMatrix[MAPROWS][MAPCOLUMNS]){
    int initialPoint[2];
    finalPoint[0] = 0;
    finalPoint[1] = 0;
    counterPath = 1;
...
bool BacktrackingAlgorithm::recursivePathing(const int initialPoint[2]) {
    ...
    //west movement
    if (((row < MAPROWS)&&(0 <= row)) && ((column - 1 < MAPCOLUMNS)&&(0 <= column - 1))) {
        ...
        //southwest movement
        if (((row + 1 < MAPROWS)&&(0 <= row + 1)) && ((column - 1 < MAPCOLUMNS)&&(0 <= column - 1))) {
        ...
        //south movement
        if (((row + 1 < MAPROWS)&&(0 <= row + 1)) && ((column < MAPCOLUMNS)&&(0 <= column))) {
